...
use crate::addr2line::function::{Functions, InlinedFunction};
use crate::addr2line::lazy::LazyCell;

pub use crate::addr2line::function::Function;
pub use crate::addr2line::function::FunctionParameter;
pub use crate::addr2line::function::Location as BaseLocation;
...
impl Context<gimli::EndianRcSlice<gimli::RunTimeEndian>> {
    ...
    pub fn new_with_sup<'data: 'file, 'file, O: object::Object<'data, 'file>>(
        ...
        Context::from_dwarf(Arc::new(dwarf))
    }
}
...
impl<R: gimli::Reader> Context<R> {
    ...
    pub fn from_sections(
        ...
        Self::from_dwarf(Arc::new(gimli::Dwarf {
            debug_abbrev,
            debug_addr,
            debug_aranges,
            debug_info,
    ...
    #[inline]
    pub fn from_dwarf(sections: Arc<gimli::Dwarf<R>>) -> Result<Self, Error> {
        let mut dwarf = ResDwarf::parse(Arc::clone(&sections))?;
    ...
    /// Internal. Find function by its linkage name
    pub fn find_by_linkage_name(&self, name: &str) -> Result<Option<Function<R>>, Error> {
        for unit in &self.dwarf.units {
            let funcs = unit.parse_functions(&self.dwarf)?;
            if let Some(f) = funcs.find_by_linkage_name(&unit.dw_unit, &self.dwarf, name)? {
                return Ok(Some(f));
            }
        }
...
pub(crate) struct ResDwarf<R: gimli::Reader> {
    ...
    fn find_function_or_location(
        ...
        let function = match functions.find_address(probe) {
            Some(address) => {
                ...
                let (offset, ref function, ref base, _) = functions.functions[function_index];
                Some(
                    function
                        .borrow_with(|| Function::parse(offset, &self.dw_unit, dwarf, base.clone()))
                        .as_ref()
                        .map_err(Error::clone)?,
                )
            }
            ...
            None => {
                let frame = Frame {
                    ...
                    function: frames.function.name.clone().map(|name| FunctionName {
                        ...
                        parameters: frames.function.parameters.params.clone(),
                        base: frames.function.base.clone(),
                    }),
                    location: loc,
                };
                self.0 = FrameIterState::Empty;
...
pub struct FunctionName<R: gimli::Reader> {
    ...
    /// Function parameters
    pub parameters: Vec<FunctionParameter<R>>,
    /// Function base addr on the stack
