...
use crate::addr2line::lazy::LazyCell;
use crate::addr2line::maybe_small;
use crate::addr2line::{Error, RangeAttributes, ResDwarf};
...
/// TODO
pub struct Function<R: gimli::Reader> {
    pub(crate) dw_die_offset: gimli::UnitOffset<R::Offset>,
    /// TODO
    pub name: Option<R>,
    pub(crate) linkage_name: Option<R>,
    ...
    /// TODO
    pub parameters: FunctionParameters<R>,
    /// TODO
...
pub enum Location {
    /// Offset added to DW_AT_frame_base
    OffsetFromBase(i64),
    /// TODO
    PlusConstant(u64),
    /// Wasm local
...
#[derive(Clone, Debug)]
/// Function parameter
pub struct FunctionParameter<R: gimli::Reader> {
    /// Parameter name
    pub name: Option<R>,
    /// Parameter location
    pub location: Option<Location>,
    /// Offset of the parameter's type
    pub type_offset: Option<gimli::UnitOffset<R::Offset>>,
}

#[derive(Clone)]
/// TODO
pub struct FunctionParameters<R: gimli::Reader> {
    /// TODO
    pub params: Vec<FunctionParameter<R>>,
}
...
impl<R: gimli::Reader> Functions<R> {
            ...
            if let Some(abbrev) = entries.read_abbreviation()? {
                if abbrev.tag() == gimli::DW_TAG_subprogram {
                                ...
                                match attr.name() {
                                    gimli::DW_AT_frame_base => match attr.value() {
                                        gimli::AttributeValue::Exprloc(data) => {
                                            base =
                                                Some(convert_dw_at_location(unit.encoding(), data));
                                        }
                                        _ => unimplemented!(),
                                    },
                                    gimli::DW_AT_linkage_name => {
                                        if let Ok(val) =
                                            dwarf.sections.attr_string(unit, attr.value())
                                        {
                                            linkage_name = Some(val);
                                        }
                                    }
                                    gimli::DW_AT_name => {
                                        if let Ok(val) =
                    ...
                    if ranges.for_each_range(&dwarf.sections, unit, |range| {
                        ...
                        let final_name = if let Some(linkage_name) = linkage_name.clone() {
                            Some(linkage_name)
                        } else {
                            name.clone()
                        };

                        functions.push((dw_die_offset, LazyCell::new(), base.clone(), final_name));
                    }
                } else {
    ...
    pub(crate) fn find_by_linkage_name(
        &self,
        unit: &gimli::Unit<R>,
        dwarf: &ResDwarf<R>,
        search: &str,
    ) -> Result<Option<Function<R>>, Error> {
        for function in &*self.functions {
            let f = Function::parse(function.0, unit, dwarf, function.2.clone())?;
            let linkage_name = if let Some(linkage_name) = f.linkage_name.as_ref() {
                linkage_name.to_string()?
            } else {
                f.name.as_ref().unwrap().to_string()?
            };
            if search == linkage_name {
                return Ok(Some(f));
            }
        }

        // for (offset, _function, _base, linkage_name) in self.functions.iter() {
        //     let linkage_name = linkage_name.as_ref().unwrap().to_string()?;
        //     if search == linkage_name {
    ...
    pub(crate) fn parse_inlined_functions(
        ...
        for function in &*self.functions {
            function
                .1
                .borrow_with(|| Function::parse(function.0, unit, dwarf, function.2.clone()))
                .as_ref()
                .map_err(Error::clone)?;
        }
        Ok(())
...
impl<R: gimli::Reader> Function<R> {
    pub(crate) fn parse(
        ...
        let mut parameters = FunctionParameters { params: Vec::new() };
    ...
    fn parse_children(
        ...
        inlined_depth: usize,
        parameters: &mut FunctionParameters<R>,
    ) -> Result<(), Error> {
        loop {
            let dw_die_offset = entries.next_offset();
                ...
                match abbrev.tag() {
                    ...
                    gimli::DW_TAG_formal_parameter => {
                        FunctionParameters::parse(
                            entries, abbrev, next_depth, unit, dwarf, parameters,
                        )?;
                    }
...
pub(crate) fn convert_dw_at_location<R: gimli::Reader>(
        ...
        match gimli::Operation::parse(&mut pc, encoding) {
            Ok(op) => match op {
                gimli::Operation::WasmLocal { index } => {
                    return Location::WasmLocal(index);
                }
                gimli::Operation::WasmGlobal { index } => {
                    return Location::WasmGlobal(index);
                }
                gimli::Operation::WasmStack { index } => {
                    panic!(" Wasm stack 0x{:x}", index);
                }
                gimli::Operation::FrameOffset { offset } => {
                    return Location::OffsetFromBase(offset);
                }
                gimli::Operation::PlusConstant { value } => {
                    return Location::PlusConstant(value);
                }
                e => panic!("{:x?}", e),
            },
            Err(err) => {
    ...
    return Location::PlusConstant(1);
    unreachable!()
}

impl<R: gimli::Reader> FunctionParameters<R> {
    fn parse(
        entries: &mut gimli::EntriesRaw<R>,
        abbrev: &gimli::Abbreviation,
        depth: isize,
            ...
            match entries.read_attribute(*spec) {
                Ok(ref attr) => match attr.name() {
                    ...
                    gimli::DW_AT_location => match attr.value() {
                        gimli::AttributeValue::Exprloc(data) => {
                            location = Some(convert_dw_at_location(unit.encoding(), data));
                        }
                        _ => {}
                    },
                    _ => {}
        ...
        parameters.params.push(FunctionParameter {
            name,
            location,
            type_offset,
        });
